...
export default function Searchbar({
  ...
  const searchHandler = (searchOption, searchKeyword) => {
    ...
    let endpoint;
    if (searchOption === '제목') {
      endpoint = `http://localhost:8080/search?titles=${searchKeyword}&search_type=titles&pages=${currentPage}&limit=10`;
    } else {
      endpoint = `http://localhost:8080/search?contents=${searchKeyword}&search_type=contents&pages=${currentPage}&limit=10`;
    }
